...
HTML;
	...
	public function recharge(){
		...
		if(isset($_POST['sososubmit'])){
			...
			if(empty($chongzhi) || $chongzhi=='请选择充值来源') $content=" AND (`content`='充值' or `content`='使用佣金充值到开心购账户' or `content`='使用余额宝充值到开心购账户')";
		...
		$num=20;
		if(empty($wheres)){
			$total=$this->db->GetCount("SELECT count(*) FROM `@#_member_account` WHERE (`content`='充值' or `content`='使用佣金充值到开心购账户' or `content`='使用余额宝充值到开心购账户') AND `type`='1'");
			$summoeny=$this->db->GetOne("SELECT sum(money) sum_money FROM `@#_member_account` WHERE (`content`='充值' or `content`='使用佣金充值到开心购账户' or `content`='使用余额宝充值到开心购账户') AND `type`='1'");
		}else{
		...
		$page->config($total,$num,$pagenum,"0");	
		if(empty($wheres)){
			$recharge=$this->db->GetPage("SELECT * FROM `@#_member_account` WHERE (`content`='充值' or `content`='使用佣金充值到开心购账户' or `content`='使用余额宝充值到开心购账户') AND `type`='1' order by `time` desc",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
		}else{
