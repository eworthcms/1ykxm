...
class pay {
	...
	private function set_dingdan($pay_type='',$dingdanzhui=''){
			...
			foreach($this->shoplist as $key=>$shop){														
					...
					$codes = $ret_data['user_code'];									//得到的开心购码
					$codes_len= intval($ret_data['user_code_len']);						//得到开心购码个数
	...
	private function pay_bag(){	
		...
		$query_4 = $this->db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '-1', '$pay_zhifu_name', '开心购了商品', '{$this->MoenyCount}', '$time')");
		$query_5 = true;	
		$query_insert = true;
		...
		if(!$fufen_dikou){
			...
			$mygoscore_text =  "开心购了{$goods_count_num}人次商品";
