				...
				<div class="GetPrize">				    
					<dl>
						...
						<dd class="gray02">
							...
							<p>开心购时间：<?php echo microt($sid_go_record['time']); ?></p>
							<p>幸运开心购码：<em class="orange Fb"><?php echo $sid_code['q_user_code']; ?></em></p>
						</dd>
					</dl>				
				</div>
			</div>
		...
		<div class="Pro_Detright">
			...
			<div class="Pro_Record">
				<ul id="ulRecordTab" class="Record_tit">
					<li class="NewestRec Record_titCur">最新开心购记录</li>
					<li class="MytRec">我的开心购记录</li>
					<li class="Explain orange">什么是1元开心购？</li>
				</ul>
				<div class="Newest_Con hide">
					<ul>
						...
						<?php endif; ?>				
						<?php echo microt($user['time']); ?> 开心购了
				...
				<div class="My_Record hide" style="display:none;">
					...
					<ul>
						...
						<li><?php echo microt($user['time']); ?>  开心购了  <?php echo $user['gonumber']; ?>  个开心购码</li>
						<?php endif; ?>
						<?php  endforeach; $ln++; unset($ln); ?>
					</ul>
					<?php  else: ?>
				...
				<div class="Newest_Con hide" style="display:none;">
					<p class="MsgIntro"><?php echo _cfg("web_name_two"); ?>购是指只需1元就有机会买到想要的商品。即每件商品被平分成若干“等份”出售，每份1元，当一件商品所有“等份”售出后，根据开心购规则产生一名幸运者，该幸运者即可获得此商品。</p>
					<p class="MsgIntro1"><?php echo _cfg("web_name_two"); ?>以“快乐开心购，惊喜无限”为宗旨，力求打造一个100%公平公正、100%正品保障、寄娱乐与购物一体化的新型购物网站。<a href="<?php echo WEB_PATH; ?>/help/1" class="blue" target="_blank">查看详情&gt;&gt;</a></p>
				</div>
			</div>			
		</div>
	</div>
...
function Cartcookie(cook){
	...
	if(cook){
		window.location.href="<?php echo WEB_PATH; ?>/member/cart/cartlist/";//+new Date().getTime()
	}
}  
</script> 
