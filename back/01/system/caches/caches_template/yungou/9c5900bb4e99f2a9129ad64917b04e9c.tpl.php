...
<div class="shop_process">
	...
	<div class="submitted">
		<ul class="order">
			...
			<li class="ts">
				<p class="right">开心购金额总计:￥<span id="moenyCount"><?php echo $MoenyCount; ?></span></p>
			</li>
		</ul>
	</div>
	<h5>
...
num.keyup(function(){
	...
	if($(this).val() > info[shopid]['shenyu']){
		message.eq(zindex).text("开心购次数不能超过"+info[shopid]['shenyu']+"次");
		$(this).val(info[shopid]['shenyu']);
		UpdataMoney(shopid,$(this).val(),zindex);		
		return;
	}
	if($(this).val()<1){
		message.eq(zindex).text("开心购次数不能少于1次");
		$(this).val(1);
		UpdataMoney(shopid,$(this).val(),zindex);
		return;
	}	
