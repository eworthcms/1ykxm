...
//福分、经验添加
	...
	public function singlelist(){
		...
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}
		$page->config($total,10,$pagenum,"0");
		//$record = $this->db->GetPage("select id from `@#_member_go_record` where shopid in ($rd_id) and `uid`='$member[uid]' and `huode`>'10000000'",array("num"=>10,"page"=>$pagenum,"type"=>1,"cache"=>0));
	...
	public function invitefriends(){
		...
		//判断哪个好友有消费
		 if(empty($accounts[$sqluid])){
		    $notinvolvednum +=1;
		    $records[$sqluid]='未参与开心购';
		 }else{
		    $involvednum +=1;
		    $records[$sqluid]='已参与开心购';
		 }
